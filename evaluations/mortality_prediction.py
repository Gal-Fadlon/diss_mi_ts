...
def main(args):
    ...
    if mode == 'supervised':
        ...
        if mode == 'glr':
            ...
            file_name = './ckpt/cdsvae%d_%d_mortality_predictor' % (args.f_dim, args.z_dim)
            # rep_model_file = './ckpt/cdsvae%d_%d_physionet' % (args.f_dim, args.z_dim)
            rep_model_file = '/home/naimani/lg_ts_tf/ckpt/cdsvae%d_%d_physionet' % (args.f_dim, args.z_dim)
            ...
            file_name = './ckpt/ours%d_%d_mortality_predictor' % (args.f_dim, args.z_dim)
            rep_model_file = './ckpt/ours%d_%d_physionet' % (args.f_dim, args.z_dim)
            encoder = Encoder(hidden_sizes=configs["cdsvae_encoder_size"])
            decoder = Decoder(output_size=configs["feature_size"],
                              output_length=configs["window_size"],
                              hidden_sizes=configs["cdsvae_decoder_size"])
            ...
            file_name = './ckpt/ours2%d_%d_%d_%f_mortality_predictor' % (args.f_dim, args.z_dim, args.ce_epoch, args.weight_f_aug)
            rep_model_file = './ckpt/ours2%d_%d_%d_%f_physionet_note=%s' % (args.f_dim, args.z_dim, args.ce_epoch, args.weight_f_aug, args.note)
            encoder = Encoder(hidden_sizes=configs["cdsvae_encoder_size"])
            decoder = Decoder(output_size=configs["feature_size"],
                              output_length=configs["window_size"],
                              hidden_sizes=configs["cdsvae_decoder_size"])
            ...
            from ours2 import OURS, Encoder, Decoder
            file_name = './ckpt/ours3%d_%d_%d_%f_mortality_predictor_note=%s' % (args.f_dim, args.z_dim, args.ce_epoch, args.weight_f_aug, "trial3")
            rep_model_file = './ckpt/ours3_fdim=%d_z_dim=%d_wuep=%d_waug=%f_wmi=%f_wrec=%f_physionet_note=%s' % \
    ...
    if args.train:
        print(' =====> Training %s ' % rep_model_file)
        if mode == 'glr':
            lr = 1e-3
        ...
        elif mode in ['cdsvae', 'ours', 'ours2', 'ours3']:
            lr = 1e-3
        ...
        for epoch in range(n_epochs+1):
            ...
            if epoch % 5 == 0:
                ...
                epoch_loss, epoch_acc, epoch_auroc = run_epoch(model, validset, rep_model, train=False, mode=mode)
    ...
    test_loss, test_acc, test_auroc = run_epoch(model, testset, rep_model, train=False, mode=mode)
    ...
    for batch_i, batch in testset.enumerate():
        ...
        if mode == 'glr':
            ...
            # print(f'z_g={z_g.shape}, z_t={z_t.shape}')
            # input('press any key...')
        elif mode == 'gpvae':
...
class Predictor(tf.keras.Model):
    ...
    def __call__(self, local_encs, global_encs, x_lens):
        ...
        # print(f'PRED FWD: h={h.shape}')
        logits = tf.keras.layers.Dropout(rate=0.5)(self.fc(h))
        probs = self.prob(logits)
        return probs


def run_epoch(model, dataset, glr_model, optimizer=None, train=False , trainable_vars=None, mode='glr'):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ce_epoch', default=0, type=int, help='warmup epochs for contrastive estimation')
