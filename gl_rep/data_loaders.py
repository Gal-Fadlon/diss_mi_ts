...
def airq_data_loader(normalize="none"):
    """Function to load the Air Quality dataset into TF dataset objects
        ...
        Args:
            normalize: The type of data normalizatino to perform ["none", "mean_zero", "min_max"]
        """
    all_files = glob.glob("/home/azencot_group/datasets/air_quality/*.csv")
    ...
    for i, sample in enumerate(df_sampled):
        ...
        if sample[0][0] in [2013, 2014, 2015, 2017]:
            ...
            2016]:  # data from 2016 is used for testing, because we have fewer recordings for the final year
            test_inds.extend([i])
    ...
    signal_maps = torch.tensor(signal_maps, dtype=torch.float32)
    z_ls = torch.tensor(z_ls, dtype=torch.float32)
...
def physionet_data_loader(normalize='none', dataset='set-a'):
    ...
    train_dataset = PhysionetDataset(signals[train_inds], maps[train_inds], signal_lens[train_inds], locals[train_inds], z_gs[train_inds])
    valid_dataset = PhysionetDataset(signals[valid_inds], maps[valid_inds], signal_lens[valid_inds], locals[valid_inds], z_gs[valid_inds])
    test_dataset = PhysionetDataset(signals[test_inds], maps[test_inds], signal_lens[test_inds], locals[test_inds], z_gs[test_inds])
